...
function TOOL:LeftClick(tr)
	...
	for i = 1, 5 do
		...
		if text ~= "" then
			for k, v in pairs(TextScreenModify.Config.BlackListWord) do 
				if string.find(text, v) then
					TextScreenModify:Notify(ply, 1, 3, "Impossible to make your text appear. A word you put in has been blacklisted!") 
					return
				end
			end 
			hasText = true
